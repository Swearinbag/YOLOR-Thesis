...
def compute_loss(p, targets, model):  # predictions, targets, model
    ...
    for i, pi in enumerate(p):  # layer index, layer predictions
        ...
        if n:
            ...
            temp = np.array(tcls[i].cpu().detach().numpy())[np.newaxis]
            temp = temp.T

            temp = np.append(temp,tbox[i].cpu().detach().numpy(),axis=1).tolist()
    ...
    loss = lbox + lobj + lcls
    #print(tgt)
    return loss * bs, torch.cat((lbox, lobj, lcls, loss)).detach(), np.array(tgt,dtype=object)
