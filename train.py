...
def train(hyp, opt, device, tb_writer=None, wandb=None):
    ...
    freeze = ['model_list.%s.' % x for x in range(100)]  # parameter names to freeze (full or partial)
    for k, v in model.named_parameters():
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        for i, (imgs, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            ...
            with amp.autocast(enabled=cuda):
                ...
                loss, loss_items,target_form = compute_loss(pred, targets.to(device), model)  # loss scaled by batch_size

                #prednp = output_to_target(pred,imgsz,imgsz).tolist()
                #targetsnp = output_to_target(targets,imgsz,imgsz).tolist()

                #prednp = pred.

                #prednp = pred.cpu().detach().numpy() #added
                #preds_list.append(pred[x,:].tolist() for x in range(len(pred))) #added

                #for x in range(len(prednp)):
                #    print(prednp[x,:])
                #    preds_list.append(prednp[x,:].tolist())
                #print(targets, targets.to(device).cpu().numpy())
                
                #if isinstance(targets, torch.Tensor):
                #    targetsnp = targets.cpu().detach().numpy()
                    #print(targetsnp)
                

                #targetsnp = targets.to(device).cpu().numpy() #added
                #targets_list.append(targetsnp[x,:].tolist() for x in range(len(targetsnp))) #added
                #for x in range(len(targetsnp)):
                #    print(targets_list[x,:])
                #    targets_list.append(targetsnp[x,:].tolist())
        ...
        # print("preds: ",len(preds_list),np.array(preds_list).shape)
        # print("targs: ",len(targets_list),np.array(targets_list).shape)

        # print(preds_list[0:5])
        # print(targets_list[0:5])

        # conf_mat.process_batch(np.array([preds_list]),np.array([targets_list]))
        # plotConfmat(conf_mat.return_matrix(),names)
        # end epoch ----------------------------------------------------------------------------------------------------
        ...
        if rank in [-1, 0]:
            ...
            if save:
                ...
                if best_fitness == fi:
                    torch.save(ckpt, best) #save when new best is present.
                if (best_fitness == fi) and (epoch >= 1200): #200
...
if __name__ == '__main__':
    ...
    if opt.resume:  # resume an interrupted run
        ...
        opt.cfg, opt.weights, opt.resume = 'cfg/yolor_p6.cfg', ckpt, True #changed '' to 'cfg/yolor_p6.cfg'
